...
6. Implement in `sessions/<codename>/worktrees/<repo>/` — not `repos/`
7. `./scripts/sync-session.sh <codename>` after other metadata changes (not needed for scope-only updates — `set-session-scope.sh` syncs index and context)
